...
def success_response(data: Any = None, message: str = "Success") -> JSONResponse:
    ...
    """
    response = APIResponse(success=True, message=message, data=data, error=None)
    return JSONResponse(status_code=200, content=response.model_dump())


def error_response(
    message: str, code: int = 400, error_details: Optional[str] = None
) -> JSONResponse:
    """
...
def setup_exception_handlers(app: FastAPI) -> None:
    ...
    async def value_error_handler(request: Request, exc: ValueError) -> JSONResponse:
        ...
        return error_response(
            message="Invalid value provided", code=400, error_details=str(exc)
        )
    ...
    async def not_found_handler(request: Request, exc: Exception) -> JSONResponse:
        ...
        return error_response(
            message="Resource not found", code=404, error_details=str(exc)
        )
