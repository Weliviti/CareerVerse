...
async def persona_chat(request: ChatRequest, user=Depends(verify_token)):
    try:
        ...
        try:
            ...
            return error_response(message=f"Persona '{request.persona_type}' not found", code=400)

        # 3. Build AI Context
        history_text = "\n".join([f"{'Student' if m.role == 'user' else 'Persona'}: {m.content}" for m in request.history])
        ...
        try:
            ...
            sim_type = "doctor" if "patient" in request.persona_type.lower() else "teacher"

            session_ref.set({
                "transcript": firestore.ArrayUnion([
                    {"role": "user", "content": request.message, "timestamp": firestore.SERVER_TIMESTAMP},
                    {"role": "assistant", "content": response_text, "emotion": emotion, "timestamp": firestore.SERVER_TIMESTAMP}
                ]),
