...
class EvaluationService:
    ...
    async def evaluate_session(self, session_id: str):
        ...
        try:
            ...
            print(f"Sending {len(transcript)} transcript entries to Gemini for evaluation...")
            ...
            skills = {
                "Communication":       raw_scores.get("communication",       raw_scores.get("Communication", 0)),
                "Empathy":             raw_scores.get("empathy",             raw_scores.get("Empathy", 0)),
                "Problem Solving":     raw_scores.get("problem_solving",     raw_scores.get("logic", raw_scores.get("Problem Solving", 0))),
                "Classroom Management": raw_scores.get("classroom_management", raw_scores.get("accuracy", raw_scores.get("Management", 0))),
            }
